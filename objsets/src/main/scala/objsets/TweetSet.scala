  ...
  def mostRetweeted: Tweet = {
    if (left.isInstanceOf[Empty] && right.isInstanceOf[Empty]) {
      ...
      if (left.isInstanceOf[Empty]) {
        ...
        val rightMostRetweeted = right.mostRetweeted
        val leftMostRetweeted = left.mostRetweeted
        val maxLeftRight = if (rightMostRetweeted.retweets >= leftMostRetweeted.retweets) rightMostRetweeted else leftMostRetweeted

        if (elem.retweets >= maxLeftRight.retweets) elem else maxLeftRight
      }
    }
  }

  def descendingByRetweet: TweetList = {
    def descendingByRetweetHelper(tweets: TweetSet): TweetList = {
      try {
        val tweet = tweets.mostRetweeted

        new Cons(tweet, descendingByRetweetHelper(tweets.remove(tweet)))
      } catch {
